...
name: impeccable
description: >-
  Designs and improves production-grade frontend interfaces. Use when the user
  wants to design, redesign, shape, critique, audit, polish, clarify, distill,
  harden, optimize, adapt, animate, colorize, extract, or improve UI — UX
  review, visual hierarchy, accessibility, theming, typography, layout, color,
  motion, and tokens. For SimplestSampler: SwiftUI macOS app UI (SamplerTheme,
  ContentView, slot rows) and AAX AudioSuite plugin GUI (aax/). Also for bland
  or loud designs needing tone adjustment, or live browser iteration. Not for
  backend-only tasks.
version: 3.5.0
user-invocable: true
...
3. Familiarize yourself with any existing design system, conventions, and components in the code. Read at least one project file (CSS / tokens / theme / a representative component or page). For SimplestSampler, start with `SimplestSampler/Views/SamplerTheme.swift` and a representative view (e.g. `ContentView.swift`, `SlotRowView.swift`); for AAX plugin UI work, read `aax/SimplestSamplerAudioSuite/Source/SimplestSampler_AS_GUI.mm`. **Required even when you've loaded a sub-command reference in step 2.** Don't reinvent the wheel; use what's there when it works, branch out when the UX wins.
